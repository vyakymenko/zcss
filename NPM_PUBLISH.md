...
`release/next-release.json` records exact candidate `0.7.0-rc.1`, tag `v0.7.0-rc.1`, npm channel `next`, and GitHub prerelease delivery under a separate fail-closed contract. Prerelease attempt `zigcss@0.7.0-rc.1` failed and its exact identity is permanently closed. GitHub surface: `absent`; npm surface: `absent`. Select a new candidate version; never move, recreate, or reuse `v0.7.0-rc.1`.

Release run [34116379683](https://github.com/vyakymenko/zigcss/actions/runs/34116379683) ended with failure on 2026-09-07: the Windows archive did not match its committed SHA-256 inventory. The four other platforms passed, but GitHub release creation, npm publication, and anonymous installation were skipped. Fresh public readback confirmed that neither release surface exists; `latest` remains `0.6.0` and `next` remains `0.6.0-rc.2`. The schema-3 contract records the successful exact-tag Build and the failed Release separately. Source-checkout capabilities remain available; no release or package is claimed for this identity.

The release workflow first revalidates the closed 0.6.0 evidence, then routes every attempted tag through the candidate admission gate. Historical tags at or below 0.6.0 are protected, permanently closed identities and cannot be admitted again. The original pre-tag policy rejected any newer tag other than `v0.7.0-rc.1` and required all seven ordered pre-tag gates, synchronized package and native-integrity identities, and an explicitly `candidate-ready` contract with `candidateReady: true`. A terminal schema-3 failure permanently supersedes those original admission conditions: they do not permit reopening the failed identity. Candidate-ready hosted and origin evidence records run IDs, timestamps, and policy results but deliberately stores no candidate commit hash, so the contract can itself be committed. At tag admission, runtime arguments bind the peeled tag commit to a fresh exact `origin/main` readback, and the workflow API gates bind same-commit Build and CodeQL results to that exact runtime commit.
...
The candidate workflow is draft-first. It rejects any published release identity; an authenticated exact-tag draft targeting the same commit may be resumed and its expected asset names reconciled with overwrite enabled. The workflow validates the exact 25 local files, uploads only that closed inventory, re-reads every GitHub asset name, size, and SHA-256 while the release is still private, and only then publishes the draft. Overwrite authority is confined to exact-tag draft reconciliation and never applies to an immutable published asset; missing or extra draft assets still fail closed. Publication must read back `immutable: true` and pass GitHub's cryptographic release-attestation verification before npm publication can start. This failed attempt created no immutable GitHub Release; a newly authorized candidate must satisfy that requirement under a new identity.
