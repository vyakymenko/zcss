...
describe('consumer package website', () => {
  ...
  test('makes the README a visual front door to the package website', () => {
    ...
    expect(readme).toContain('**Stable package identity: 0.6.0 — published.**')
    expect(readme).toContain('`zigcss@0.6.0` is an immutable npm version')
    expect(readme).toContain('npm install --save-dev zigcss@0.6.0')
    expect(readme).toContain('This README records a versioned contract, not a live publication status.')
    expect(readme).toContain('https://github.com/vyakymenko/zigcss/releases/tag/v0.7.0-rc.1')
    expect(readme).toContain('https://www.npmjs.com/package/zigcss/v/0.7.0-rc.1')
    expect(readme).toContain('After confirming that the exact prerelease version is published in the [live release status]')
    expect(readme).toContain('npm install --save-dev zigcss@0.7.0-rc.1')
    expect(readme).not.toMatch(/npm `(?:latest|next)` (?:still )?serves/)
  ...
  test('documents package-manager-neutral lifecycle recovery without claiming offline downloads', () => {
    ...
    expect(readme).toContain('The `0.7.0-rc.1` prerelease package contract adds both of those surfaces.')
    expect(readme).toContain('Its `native-integrity.json` is an exact versioned SHA-256 inventory for all five native archives.')
    expect(readme).toContain('require that digest to match the independent digest carried through npm')
    expect(readme).not.toContain('No published npm version currently exposes')
