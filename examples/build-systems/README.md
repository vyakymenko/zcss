...
CI explicitly installs all four tools from the runner's authenticated Ubuntu package repositories and checks their availability before the native test suite. The later full integration gate still requires clean, no-op, and dependency-only rebuilds through the exact current-checkout compiler. Distribution package acquisition is networked and is not a content-addressed toolchain pin.
