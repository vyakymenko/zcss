...
> **Stable package identity: 0.6.0 — published.** `zigcss@0.6.0` is an immutable npm version, and the matching non-prerelease [GitHub Release](https://github.com/vyakymenko/zigcss/releases/tag/v0.6.0) carries 25 verified release files: 15 attested archive/checksum/SBOM subjects and 10 Sigstore bundle files. Historical npm version `0.6.0-rc.2` is also preserved by its exact version. Both historical GitHub releases predate Immutable Releases and have recorded `immutable: false` readbacks; neither is evidence of immutable GitHub publication for a later version.

> **Prerelease package identity: 0.7.0-rc.1.** This version extends the native CLI with dependency files, target prefixing, all-syntax optimizer and Source Map guarantees, hardened output commits, and deterministic cross-platform batch naming. It also adds a typed programmatic Node.js API and `zigcss-install` recovery entry point at the package root; experimental adapters for Vite, Rollup, esbuild, Bun, Webpack, and Rspack; bounded Next.js 16.3.4 Turbopack and Webpack host proofs that reuse `zigcss/webpack`; pinned SvelteKit 2.70.3, Astro 7.2.10, and Nuxt 4.5.2 external CSS Module examples that reuse `zigcss/vite`; and a real local-transformer Parcel example. None of these additions rewrites the published 0.6.0 artifact. Source-checkout use requires building that checkout's native binary. The distinct prerelease identity prevents this package from masquerading as stable `zigcss@0.6.0`; package installation requires matching, integrity-verified native archives for the exact version.

This README records a versioned contract, not a live publication status. Check the [exact-version GitHub Release](https://github.com/vyakymenko/zigcss/releases/tag/v0.7.0-rc.1), [exact npm version](https://www.npmjs.com/package/zigcss/v/0.7.0-rc.1), and maintained [release status](https://vyakymenko.github.io/zigcss/docs/guide/status/) for availability and verification results. The [live release contract](https://github.com/vyakymenko/zigcss/blob/main/release/next-release.json) tracks admission and publication separately. A prerelease does not promote the stable channel; check those live sources before selecting a version or dist-tag.
...
The native implementation was first proven on protected historical tag `v0.6.0-rc.2`: its machine contract records `nativeReleaseReady: true`, and all nine pre-tag evidence surfaces, five attested target archives, offline consumers, and provenance passed. Its historical GitHub prerelease and npm publication are verified; GitHub prerelease 369856953 has a recorded `immutable: false` readback, while npm version `0.6.0-rc.2` is immutable. Stable `0.6.0` then passed its separate ten-gate terminal and was published from protected tag `v0.6.0`; GitHub Release 372291445 also has a recorded `immutable: false` readback, while npm version `0.6.0` is immutable.
...
Install the exact published stable five-language baseline:
...
```bash
npm install --save-dev zigcss@0.7.0-rc.1
```
...
The remaining CLI examples describe the `0.7.0-rc.1` prerelease contract, not the immutable stable 0.6.0 binary. For source-checkout use, switch this checkout back to `main` and rebuild before running them:
...
The `0.7.0-rc.1` prerelease package exports a typed programmatic Node.js API from its package root for both CommonJS and real ESM consumers. It provides `compile`, `compileSync`, `compileFile`, `compileFileSync`, `detectSyntax`, and `ZigCssCompileError`, with declarations from `api.d.ts`. In a direct source checkout, first run `zig build -Doptimize=ReleaseFast`; the API selects only the regular, non-symlink `zig-out/bin/zigcss` when the exact source markers are present. Otherwise it requires the installed package's regular executable. Every compile call starts one private native process without a shell or temporary source file; the package retains zero production and optional dependencies.

The published stable 0.6.0 npm package is not compatible with this API because its immutable binary has no `--internal-node-v1` protocol. The distinct `0.7.0-rc.1` prerelease identity cannot masquerade as that stable package. These JavaScript surfaces require the matching native protocol and exact-version archives; use the release-status links above to verify delivery availability.
